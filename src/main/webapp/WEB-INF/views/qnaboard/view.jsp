...
function deleteComment(board_ref, comm_idx) {
	if (confirm("댓글을 삭제하시겠습니까?")) {
		...
		form.method = "post";
		form.action = "/qnaboard/deleteComment.do";
		form.submit();
	}
}
...
<body>
			...
			<div class="board_inner">
				...
				<div class="board_btn">
					...
					<c:if test="${ boardDTO.writer_ref == sessionScope.userId }">
						<button type="button" onclick="location.href='../qnaboard/editPost.do?board_idx=${ param.board_idx }';">수정하기</button>
				...
				<div class="like_btn">
					...
					<c:if test="${ likecheck == 0 }">
						<button type="button" onclick="location.href='../qnaboard/clickLike.do?board_idx=${ param.board_idx }';">
							<p class="like"></p>
							<span>${ likecount }</span>
						</button>
					</c:if>
					...
					<c:if test="${ likecheck == 1 }">
						<button class="push" type="button" onclick="location.href='../qnaboard/clickLike.do?board_idx=${ param.board_idx }';">
							<p class="like"></p>
							<span>${ likecount }</span>
						</button>
					</c:if>
					...
					<c:if test="${ reportcount == 0 }">
						<button type="button" onclick="location.href='../qnaboard/clickReport.do?board_idx=${ param.board_idx }';">
							<p class="dislike"></p>
					...
					<c:if test="${ reportcount == 1 }">
						<button class="push" type="button" onclick="location.href='../qnaboard/clickReport.do?board_idx=${ param.board_idx }';">
							<p class="dislike"></p>
							<span>${ reportcount }</span>
						</button>
					</c:if>
    ...
    <!-- 댓글 작성 모달창 -->
	<form method="post" action="../qnaboard/writeComment.do" onsubmit="return validateCommentForm(this);">
	...
	<!-- 댓글 수정 모달창 -->
	<form method="post" action="../qnaboard/editComment.do" onsubmit="return validateCommentForm(this);">
