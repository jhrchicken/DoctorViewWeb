...
🔵 프로젝트 구조 만들었습니다<br/>
🔵 '이슈 및 문제해결'에서 다영이가 적은 부분 '문제가 있었습니다'라고 끝나는게 개수상함 너뭐얏ㅡㅡ << 카톡으로 보내준 내용보고 수정해서 적었습니닷 읽어보고 뿌영님이 의도한 내용이 아니면 수정 부탁드려요<br/>
...
🔵 개선점이랑 향후계획 같이 묶기로 했었는데 다시보니까 좀 어색해서 개선점은 앞으로 빼고, 결과 및 확장성 마지막에 향후계획 두 문장 추가했어요
...
병원 목록 구현 과정에서 기존의 요청 기반 페이징 방식과 AJAX 기반 필터링 방식 간의 충돌로 인해, 필터링된 목록에서 페이지 이동 시 전체 병원을 기준으로 페이징이 적용되는 문제가 발생했습니다. 페이징 기능을 제거하는 경우 필터링 조건을 해제할 때 약 25,000건의 데이터를 한 번에 조회해야 하여 성능이 크게 저하되었습니다.
...
현재 병원 회원 가입 시, 서로 다른 세 개의 테이블에 데이터가 삽입됩니다. 그러나 데이터 삽입 중 하나라도 오류가 발생하면 회원 가입이 불가능해야 합니다. 현재는 JavaScript를 통해 사용자 데이터 전송만 제한되어 있을 뿐, DB에서의 데이터 무결성 보장이 부족합니다. 따라서 DB 측에서 트랜잭션을 활용하여 병원 회원 데이터 삽입 과정을 하나의 작업 단위로 묶어, 데이터 무결성을 보장하도록 개선해야 합니다.

좋은 평점, or 사용자 피드백을 받아 이전보다 더 나아진 평을 받은 병원의 순위(?)를 올려 어드밴티지 부여 > 병원서비스 향상

병원 정보 실제여부에 대한 신고를 받아 정보를 최신으로 업데이트하지않으면 안내(경고)를 하여 최신정보로 업데이트 유도

현재 예약 close만 가능 > open도 가능하도록
            
현재 예약 한건이 들어오면 close됨 > 한 타임 당 예약을 몇건 받을 것인지 병원으로부터 입력받을 수 있도록
...
프로젝트를 시작할 때는 제 부족함과 실력에 대한 막연한 두려움이 있었습니다. 하지만 프로젝트를 진행하면서, 배우려는 자세와 의지만 있다면 결과와 상관없이 저를 더 단단하게 만들어주는 실력과 경험을 얻을 수 있음을 깨달았습니다.

앞으로도 백엔드 개발자를 향해 달려갈수록 제가 모르는 기술들은 끝없이 쏟아져 나올 것입니다. 그 사실이 두렵기도 하고 때로는 자신감을 잃기도 하겠지만 차근히 공부하다보면 꾸준히 성장하게 될 것입니다. 결국 여정 자체가 저에게 가장 큰 보상이 된다는 것을 깨닫게 되었습니다.
